...
- 生成中文提示词，包含 CriticMarkup 规则、处理要求、当前 Markdown 原文文件地址，以及“直接修改文件、不新增修订标记、保留原批注”的约束。
- 支持复制到剪贴板，或在新编辑器中打开提示词。
- 提示词不依赖外部 JSON 批注文件，也不内嵌 Markdown 原文全文；修改差异由 Git diff 承担。

验收标准：

- 用户可以一键得到可直接交给 Codex 的中文提示词。
- 提示词明确要求 AI 直接修改 Markdown 文件，不通过 CriticMarkup 表示新增、删除和替换。
- 提示词明确说明如何接受已有新增、删除、替换建议，以及如何保留原批注。
- 提示词强调保留标题、列表、代码块、链接等 Markdown 结构，并保留原有批注意见。
